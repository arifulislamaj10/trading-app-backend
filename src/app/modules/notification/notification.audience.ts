 ...
 */
export const audienceFromLegacyTargetRole = (
  targetRole?: string,
): BroadcastAudience => {
  if (!targetRole) {
    return { type: "ALL" };
...
export const resolveAudienceRecipients = async (
  ...
  const filterActiveAccounts = async (
    accountIds: string[],
  ): Promise<string[]> => {
  ...
  switch (audience.type) {
    case "ALL": {
      const accounts = await Account_Model.find(ACTIVE_ACCOUNT_FILTER).select(
        "_id",
      );
      return accounts.map((a) => a._id.toString());
    }
    ...
    case "SUBSCRIPTION_TIER": {
      if (!audience.tier) {
        throw new AppError(
          "tier is required for SUBSCRIPTION_TIER audience",
    ...
    case "ACTIVE_SUBSCRIBERS": {
      const { Subscription_Model } =
        await import("../subscription/subscription.schema");
    ...
    case "FOLLOWERS_OF_MASTER": {
      if (!audience.masterId || !Types.ObjectId.isValid(audience.masterId)) {
        throw new AppError(
          "Valid masterId is required for FOLLOWERS_OF_MASTER audience",
          httpStatus.BAD_REQUEST,
        );
      }

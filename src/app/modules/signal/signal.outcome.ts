 ...
 */
export const resolveSignalOutcome = (source: SignalOutcomeSource): SignalOutcome => {
  ...
  $cond: [
    {
      $or: [
        ...
        {
          $and: [
            { $not: [{ $in: ['$outcome', ['hit_target', 'stopped_out', 'cancelled']] }] },
            { $eq: ['$status', 'won'] },
          ],
        },
        {
...
export const formatSignalOutcomeLabel = (outcome: SignalOutcome | string): string => {
